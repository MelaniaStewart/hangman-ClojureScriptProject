...
;;(def Bootstrap-Button (reagent/adapt-react-class (aget js/ReactBootstrap "Button")))


;; FUNCTIONS - NON COMPONENT
;;(defn do-letter [letter] [:label (str letter "&nbsp;")])
...
;;(def list-of-jpegs ["one.jpg" "two.jpg" "three.jpg" "four.jpg" "five.jpg" "six.jpg" "seven.jpg" "eight.jpg" "nine.jpg" "ten.jpg"])
...
(defn perform-restart []
      (let [a (reagent/atom "")
           ...
           ;;(js/alert @max-number-of-failures)
           ;;(js/alert @number-of-failures)
           #_(if (= @max-number-of-failures @number-of-failures)
           ...
           (reset! new-word @a)
           (reset! status-info "You have 6 tries.")
           ;;(reset! tmp  @status-info)
           )
      )

(defn perform-restart-complete []
      ;;(reset! status-info (str "Previous -> " @new-word))
      (perform-restart)
...
(defn limit-to-one-letter
      ...
      (let [entry (apply str (re-seq #"[a-zA-Z]" (aget (js/document.getElementById "Hangman-By-Text") "value")))]
           ;;(aget (js/document.getElementById "Hangman-Tries") "value" )
           (if (= (count entry) 1)
...
(defn hangman-tries-div
      []
      [:div
       ""
       [:label {:for "Hangman-Tries"
                } "Number of Tries:"]
       [:input {:id      "Hangman-Tries"
                :type    "text"
                :maxsize "2"
                :size    "2"
                :value   @max-number-of-failures
                }]])

(defn guess-by-text-div
      []
      [:div
       ""
       [:label {:for "Hangman-By-Text"} "Type a letter:"]
       [:input {:id        "Hangman-By-Text"
                :type      "text"
                :maxsize   "1"
                :size      "1"
                :style     {:text-transform "uppercase"}
                :on-change #(limit-to-one-letter)
                :value     @letter-from-text
                }]])
